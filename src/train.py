...
# compute path to data directory relative to this script file
# this ensures the same folder is used whether you run `python train.py`
# from the root or from the src directory.
BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
dataset_path = os.path.join(BASE_DIR, 'train_data', 'data')
...
def load_dataset(feature_set='raw', dir_names=[]):
    ...
    for dir_name in dir_names:
        print(dir_name)
        # support .JPG, .jpg, .JPEG, .jpeg .png, .PNG extensions
        imgs = glob(f'{dataset_path}/{dir_name}/*.png')
        imgs += glob(f'{dataset_path}/{dir_name}/*.PNG')
        imgs += glob(f'{dataset_path}/{dir_name}/*.jpg')
        imgs += glob(f'{dataset_path}/{dir_name}/*.JPG')
        imgs += glob(f'{dataset_path}/{dir_name}/*.jpeg')
        imgs += glob(f'{dataset_path}/{dir_name}/*.JPEG')
        count += len(imgs)
        if len(imgs) == 0:
            # no examples found for this class; warn the user
            print(f"  (warning: no images found in class '{dir_name}'")
...
def train(model_name, feature_name, saved_model_name):
    # each entry returned by glob is the full path to a class folder
    # use os.path.basename to get just the folder name regardless of path
    dir_names = [os.path.basename(path) for path in glob(f'{dataset_path}/*')]
