...
def estim(c, idx, imgs_spacing, imgs_rows):
    ...
    # Aumentamos un poco el margen para partituras con ruido o inclinación
    margin = spacing / 3 
    
    # DEBUG: Para ver si el programa detectó las 5 líneas
    if len(rows) < 5:
        print(f" ¡Cuidado! Solo detecté {len(rows)} líneas en la sección {idx}")

    for index, line in enumerate(rows):
        # Si la nota está muy cerca de la línea (Nota sobre línea)
        if abs(c - line) <= margin:
            return index + 1, 0
            
        # Si la nota está entre esta línea y la siguiente (Nota en espacio)
        if index < len(rows) - 1:
            next_line = rows[index + 1]
            if line < c < next_line:
                return index + 1, 1

    # ESTRATEGIA DE RESCATE: Si llega aquí, buscamos la línea más cercana 
    # en lugar de devolver siempre 'c1'.
    distances = [abs(c - line) for line in rows]
    closest_line_idx = distances.index(min(distances))
    
    # Si la nota está por debajo de la última línea
    if c > rows[-1]:
        return 7, 1 
    
    # Si está por encima de la primera, devolvemos la posición más alta
    return closest_line_idx + 1, 0
...
def get_note_name(prev, octave, duration):
    ...
    if '4' in duration:
        ...
        # Si sigue fallando, que nos diga qué etiqueta exacta está dando la IA
        return f"ERROR_{duration}_{octave}"
...
def recognize(out_file, most_common, coord_imgs, imgs_with_staff, imgs_spacing, imgs_rows):
    ...
    for i, img in enumerate(coord_imgs):
        ...
        for j, prim in enumerate(primitives):
            print(f"Processing primitive {j} in image {i}")
            ...
            print(f"Predicted labels for primitive {j} in image {i}: {labels}")
            octave = None
            label = labels[0]
            if label in black_names or True:
                print(label)
